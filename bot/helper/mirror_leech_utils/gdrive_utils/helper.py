...
GDRIVE_SERVICE_ACCOUNTS_PATH = "accounts"


class GoogleDriveCredentialError(ValueError):
    """Raised when an OAuth token must be regenerated by the user."""


def is_gdrive_auth_error(error):
    """Return whether retrying cannot repair this Google authorization."""
    current = error
    for _ in range(4):
        if isinstance(current, (GoogleDriveCredentialError, RefreshError)):
            return True
...
def should_retry_gdrive_error(error):
    """Retry transient Drive failures, never permanent auth or missing files."""
    if is_gdrive_auth_error(error):
        return False
    return getattr(getattr(error, "resp", None), "status", None) != 404


def gdrive_auth_error_message(token_path):
    target = (
        "Generate a new token with /tokengen, then replace TOKEN.PICKLE "
        "in /userset > GDrive Tools."
        if token_path.startswith("tokens/")
        else "The bot owner must regenerate and replace token.pickle."
    )
    return f"Google Drive authorization expired or was revoked. {target}"


def _save_refreshed_credentials(token_path, credentials):
    """Persist refreshed OAuth state atomically for subsequent tasks."""
    temporary_path = ""
    try:
        token_dir = ospath.dirname(ospath.abspath(token_path))
        with NamedTemporaryFile(
            mode="wb",
            dir=token_dir,
            prefix=".amaterasu-token-",
            suffix=".tmp",
            delete=False,
        ) as temporary:
            temporary_path = temporary.name
            pdump(credentials, temporary)
        replace(temporary_path, token_path)
    except OSError as error:
        LOGGER.warning("Unable to persist refreshed Google credential: %s", error)
        if temporary_path and ospath.exists(temporary_path):
            try:
...
def validate_gdrive_oauth_token(token_path):
    """Refresh an expired OAuth token before a download task is started."""
    helper = GoogleDriveHelper()
    helper.use_sa = False
    helper.token_path = token_path
    helper.authorize(validate=True)


class GoogleDriveHelper:
    def __init__(self):
        self._OAUTH_SCOPE = ["https://www.googleapis.com/auth/drive"]
        self.token_path = "token.pickle"  # noqa: S105 - credential file path
    ...
    def authorize(self, validate=False):
        ...
        if self.use_sa:
            ...
            with open(self.token_path, "rb") as f:
                credentials = pload(f)
            if validate and not getattr(credentials, "valid", False):
                ...
                _save_refreshed_credentials(self.token_path, credentials)
        else:
