"""Phase 1.2 — Protected.link uploader.

Uploads files to Protected.link via their API.
Requires Config.PROTECTED_API to be set. If not set, the upload
...
class ProtectedUpload:
    ...
    def __init__(self, listener, path):
        ...
        self.api_key = user_dict.get("PROTECTED_API") or Config.PROTECTED_API
    ...
    async def _upload_file(self, session, file_path):
        if not self.api_key:
            raise Exception("Protected.link API key not configured")
        self.__processed_bytes = 0
        self.last_uploaded = 0
            ...
            with open(file_path, "rb") as f:
                async with session.post(url, data=data, files={"file": f}) as resp:
                    ...
                    if result.get("status") != 200:
                        raise Exception(
                            f"Protected.link upload failed: {result.get('msg', 'unknown error')}"
                        )
                    download_url = result.get("result", {}).get("url", "")
                    return download_url
        except Exception as e:
            LOGGER.error(f"Protected.link upload error: {e}")
            raise
    ...
    async def upload(self):
        if not self.api_key:
            await self.listener.on_upload_error(
                "Protected.link API key not configured. Set PROTECTED_API in config."
            )
            return
        ...
        async with ClientSession() as session:
            try:
                ...
                await self.listener.on_upload_error(f"Protected.link upload failed: {e}")
    ...
    async def cancel_task(self):
        ...
        if self._updater is not None:
            self._updater.cancel()
        await self.listener.on_upload_error("Protected.link upload cancelled")
