"""Phase 1.2 — StreamWish uploader.

Uploads files to StreamWish (streamwish.com) via their API.
Requires Config.STREAMWISH_API to be set. If not set, the upload
fails silently with a clear log message (Rule 2 — no UI noise).

StreamWish API docs: https://streamwish.com/api
Upload endpoint: POST /api/v2/upload
"""
...
class StreamWishUpload:
    ...
    def __init__(self, listener, path, folder_name=""):
        ...
        self.api_key = user_dict.get("STREAMWISH_API") or Config.STREAMWISH_API
    ...
    async def _upload_file(self, session, file_path):
        """Upload a single file to StreamWish."""
        if not self.api_key:
            raise Exception("StreamWish API key not configured")
        file_size = Path(file_path).stat().st_size
        self.__processed_bytes = 0
        self.last_uploaded = 0

        # StreamWish upload API
            ...
            with open(file_path, "rb") as f:
                async with session.post(url, data=data, files={"file": f}) as resp:
                    ...
                    if result.get("status") != 200:
                        raise Exception(
                            f"StreamWish upload failed: {result.get('msg', 'unknown error')}"
                        )
                    file_code = result.get("result", {}).get("filecode")
                    download_url = f"https://streamwish.com/{file_code}"
                    return download_url
        except Exception as e:
            LOGGER.error(f"StreamWish upload error: {e}")
            raise
    ...
    async def _upload_dir(self, input_directory):
        ...
        for root, _, files in oswalk(input_directory):
            for file in files:
                ...
                await self._upload_file(None, file_path)  # session handling per-file
                self.total_files += 1
    ...
    async def upload(self):
        ...
        if not self.api_key:
            await self.listener.on_upload_error(
                "StreamWish API key not configured. Set STREAMWISH_API in config."
            )
            return
        ...
        async with ClientSession() as session:
            try:
                ...
                await self.listener.on_upload_error(f"StreamWish upload failed: {e}")
    ...
    async def cancel_task(self):
        ...
        if self._updater is not None:
            self._updater.cancel()
        await self.listener.on_upload_error("StreamWish upload cancelled")
