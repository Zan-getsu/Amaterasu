...
async def _remove_job(nzo_id, mid=None):
    ...
    for result in results:
        if isinstance(result, BaseException):
            LOGGER.error(f"{result}: Sabnzbd, while removing job. ID: {nzo_id}")
    if isinstance(results[0], BaseException) or not results[0]:
        try:
            await sabnzbd_client.delete_job(nzo_id, True)
        except Exception as e:
            LOGGER.error(f"{e}: Sabnzbd, while deleting job. ID: {nzo_id}")
    async with nzb_listener_lock:
