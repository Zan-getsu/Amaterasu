...
async def restart_bot(_, message):
    ...
    buttons.data_button(
        "Yes!", f"botrestart confirm {mode}", style=ButtonStyle.SUCCESS
    )
...
async def restart_sessions(_, message):
    buttons = button_build.ButtonMaker()
    buttons.data_button(
        "Yes!", "sessionrestart confirm", style=ButtonStyle.SUCCESS
    )
    buttons.data_button(
        "No!", "sessionrestart cancel", style=ButtonStyle.DANGER
    )
    button = buttons.build_menu(2)
    await send_message(
        message,
