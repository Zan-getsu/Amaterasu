...
from ..core.cloudflare_tunnel import cloudflare_tunnel_booter
...
def _config_update_payload(key, value):
    data = {key: value}
    if key == "BASE_URL":
        data.update(
            {
        ...
        data["CLOUDFLARE_TUNNEL_AUTO_FQDN"] = None
    return data
...
DEFAULT_DESP = {
    ...
    "CLOUDFLARE_TUNNEL_METRICS": "Local cloudflared metrics listener. Default: 127.0.0.1:49312.",
    "CLOUDFLARE_TUNNEL_AUTO_URL": "For quick tunnels, auto-fill BASE_URL from the generated trycloudflare.com URL.",
    "DEFAULT_LANG": "Default bot language code. Default: en.",
    "DATABASE_URL": "MongoDB connection string for persistent storage.",
    "DEFAULT_UPLOAD": "Default upload destination: gd (Google Drive), rc (rclone), or mega.",
    ...
    "DIRECT_PARALLELISM": "Concurrent files per multi-file direct/index task. Valid range: 1-16. Default: 4.",
    "MEGA_LIMIT": "Mega download size limit in GB. 0 = unlimited.",
    "TERABOX_ENABLED": "Enable native TeraBox share and account downloads.",
    "TERABOX_LIMIT": "TeraBox download size limit in GB. 0 = unlimited.",
    ...
    "UPLOAD_PARALLELISM": "Concurrent files per Telegram leech task, including metadata probing and upload. Valid range: 1-16. Default: 3.",
    ...
    "PROGRESS_BAR": "Progress bar symbols. Use FILLED:EMPTY format, e.g. █:░ or 🟩:⬛.",
    ...
    if key == "DISABLE_JD":
        ...
        if disabled:
            if sabnzbd_client.LOGGED_IN:
                ...
                try:
                    await (
                        await create_subprocess_exec("pkill", "-9", "-f", "SABnzbd")
                    ).wait()
                except Exception:
                    pass
