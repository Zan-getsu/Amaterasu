...
<ul>{kw_items}</ul>
        ...
        if template:
            ...
            try:
                ...
                LOGGER.warning(
                    "Telegram rich IMDb peer resolution failed; using text: %s",
                    e,
                )
                await send_message(reply_to, cap, buttons)
                await delete_message(message)
                return
            rich_files = []
            ...
            if len(rich_files) == 1:
                ...
                slides = "\n".join(
                    f'<img src="tg://photo?id={item.id}"/>' for item in rich_files
                )
...
<a href="{url}">Open on IMDb</a>"""
            ...
            try:
                ...
                LOGGER.warning("Telegram rich IMDb result failed; using text: %s", e)
                await send_message(reply_to, cap, buttons)
        await delete_message(message)
    else:
        await query.answer()
