...
RUN /amaterasuvenv/bin/uv pip install --python /amaterasuvenv/bin/python --no-cache-dir -r requirements.txt \
    && /amaterasuvenv/bin/python -c "from importlib.metadata import version; import pyrogram, warpcrypto; from pyrogram.crypto import aes; assert pyrogram.__version__ == '3.1.0', pyrogram.__version__; assert version('warpcrypto') == '2.0.7', version('warpcrypto'); assert getattr(aes, 'warpcrypto', None) is warpcrypto, 'WarpCrypto is not WZGram active crypto backend'; print('Verified WZGram 3.1.0 with active WarpCrypto 2.0.7 backend')"
